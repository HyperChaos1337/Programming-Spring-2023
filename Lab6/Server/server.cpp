...
void Server::initServer() {
    ...
    statusLabel->setText(tr("Сервер успешно запущен\n\nIP-Адрес: %1\nНомер порта: %2\n\n")
                             .arg(ipAddress).arg(tcpServer->serverPort()));
    db = QSqlDatabase::addDatabase("QPSQL");
    db.setDatabaseName("LastLab");
    db.setPort(5432);
    db.setPassword("ChaosNova2020");
...
void Server::sendInsertData(const QString &valueF, const QString &valueN, const QString &valueGroup,
    ...
    QSqlQuery query(db);
    QString column;
    if (query.exec("INSERT INTO student(_Surname,_Name,_Country,_Gender) VALUES("
